...
isUnspaced(unspacedStr, words, idx, max:=4) {
    ; I cbf trying to replace every item name with spacing ngl, so there's this function instead.
    word := words[idx]
    text := word.Text
    Loop max {
        if text = unspacedStr {
            return [true, A_Index - 1]
        }
        nextWord := (idx + A_Index <= words.Length) ? words[idx + A_Index] : ""
        if !nextWord {
            return [false, false]
        }
        text .= nextWord.Text
 ...
 * @param {Integer, String, Bitmap, OCRResult} x X position of rect, or Bitmap, OCR Result or File Path to an img
 * @param {Integer, Func} y Y position of rect, Func to filter words.
 * @param {Integer, Func} w Width of rect, Func to check words
 * @param {Integer} h Height of rect
 * @param {Integer, Map} options int or OCR options
 * @param {Func} filterWords Function to return words to iterate through
 * @param {Func} customCheck Function to return true if correct word is found
**/
findTextInRect(str, x, y?, w?, h?, options:=1, filterWords?, customCheck?) {
    ...
    if IsSet(customCheck) {
        ...
        for idx, word in words {
            if isUnspaced(unspacedStr, words, idx, replacements + 4)[1] {
                searchResult["Word"] := word
                break
            }
        }
...
class OCR {
    ...
    static FromFile(FileName, Options:=0) {
        ...
        IRandomAccessStream := this.HBitmapToRandomAccessStream(hBM)
    ...
    static LoadLanguage(lang:="FirstFromAvailableLanguages") {
        ...
        if (OcrEngine.ptr = 0)
            Throw Error(lang = "FirstFromAvailableLanguages" ? "No OCR supporting languages are installed on your system! Please follow the Knowledge Base guide to install a supported language as a secondary language on Windows." : "Can not use language `"" lang "`" for OCR, please install language pack.")
